...
staging_dir=$(mktemp -d "$temp_dir/redscript-archive.XXXXXXXX")
...
# Create a fresh ZIP so repeated packaging cannot retain removed entries.
(cd "$staging_dir" && zip -r package.zip *)
mv -f "$staging_dir/package.zip" "$working_dir/$archive_name"
